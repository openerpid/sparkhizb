...
namespace App\Hizb\Builder\Syshab;
...
class WorkorderBuilder
{
    ...
    private function sjQuery()
    {
        ...
        $subquery = $this->db->table('tr_machine_conditionh a')
            ...
            ->select("
                a.trans_id as id,
                a.trans_id as order_number,
                a.unit_code as unit_kode,
                a.trans_status as status_id,
                b.model_no as unit_model,
                c.wo_desc AS status_name,
                a.*
            ")
            ->join('ms_unit b', 'b.unit_code = a.unit_code', 'left')
            ->join('ms_wo_status c', 'c.wo_status = a.trans_status', 'left')
            ->where('a.void <> 4');
